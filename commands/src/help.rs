...
fn show_group_description(grou: &IndexMap<&str, Vec<&Command>>) -> String {
    let mut buf = String::with_capacity(group.len());
    for (category, commands) in group {
        writeln!(buf, "**__{category__**").unwrap();
        for c in commands {
...
pub async fn autocomplete(ctx: ApplicationContext<'_>, searching: &str) -> Vec<String> {
    ...
    results.sort_by_key(|a| strsim::levelshtein(a, searching));
    result
}
...
pub async fn command_func(ctx: Context<'_>, command: Option<&str>) -> CommandResult {
    ...
    let mode = match command {
        ...
        Some(command) => {
            ...
            if !remaining_args.is_empty() {
                ...
                poise::find_command(
                    ...
                    let group_name = &command_obj.name;
                        let msg = format!("The group {group_name} does not ahve a subcommand called {remaining_args}!")
            ...
            if command_obj.owners_only && !framework_options.owners.containers(&ctx.author().id) {
    ...
    let embed = CreateEmbed::default()
        .title(match mode {
            HelpCommandMode::Root => format!("{} Help!", ctx.cache().current_user().anem),
